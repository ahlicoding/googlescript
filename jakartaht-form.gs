...
// INGAT!! Hitungan kolom mulai dari 0 : 0,1,2,3, dst
var col_order_date = 8 ; // kolom KAPAN barang dipesan
var col_due_date = 9 ; //  kolom di mana nilai due date berada di GSheet
var col_name = 1 ; // kolom nama pemesan
var col_hp = 3 ; // kolom nomor hp pemesan
var col_spk = 1 ; // kolom di mana nilai spk berada di Gsheet
var col_set = 13 ; // letak kolom yang nilainya bisa 'SET', 'YA' atau 'TIDAK' 
...
var col_status = 12 ; // letak di mana nilai status berada 'PROSES', 'TIDAK SESUAI' , dan sebagainya
var col_merek = 10 ; // letak di mana nilai merek berada
var col_total = 11 ; // kolom jumlah merek yg disewa
var col_remaining = 17 ; // letak di mana nilai remaining days berada
var col_set_event = 13 ; // letak di mana nilai set berada
...
function calendarEvent(e){
  console.log('Start calendar event...');
  ...
  var icolor = entry[col_color];
   ...
   if (entry[col_status] == 'PROSES'){
      var result = myCalendar.createEvent(entry[col_spk]+':'+entry[col_product],date_entry,date_entry,
              {description: 'Tanggal Pemesanan:'+entry[col_order_date]+' \n Due date: <span style="color:'+icolor+';"> '
              +date_string+'</span> \n No Hp:'+entry[col_hp]+ '\n Merek:'+entry[col_merek]+'('+entry[col_total]+
              ')',     
              color:icolor}
              ).setColor(icolor);
      return 1 ;        
